...
local items = {
    settingsfile = "destruction_by_makaren.json",
...
local function OnLoad()
	ni.GUI.AddFrame("destruction_by_makaren", items);
end;
local function OnUnLoad()  
	ni.GUI.DestroyFrame("destruction_by_makaren");
end;
local cache = {
    IsMoving = false,
    stopcast = false,
...
|cFFFF0000=========================|r")
	...
	ni.bootstrap.profile("destruction_by_makaren", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = {
        "Error",
    };
    ...
    ni.bootstrap.profile("destruction_by_makaren", queue, abilities);
end;