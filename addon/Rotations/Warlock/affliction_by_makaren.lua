...
local items = {
    settingsfile = "affliction_by_makaren.json",
...
local function OnLoad()
	ni.GUI.AddFrame("affliction_by_makaren", items);
end;
local function OnUnLoad()  
	ni.GUI.DestroyFrame("affliction_by_makaren");
end;
local cache = {
    IsMoving = false,
    stopcast = false,
...
|cFFFF0000=========================|r")
	...
	ni.bootstrap.profile("affliction_by_makaren", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = {
        "Error",
    };
    ...
    ni.bootstrap.profile("affliction_by_makaren", queue, abilities);
end;