...
local items = {
    ...
    {
        type = "entry",
        text = "|cFFFF0000\124T" .. select(3, GetSpellInfo(spells.FerociusBite.id)).. ":26:26\124t Full Automated rotation|r",
...
local abilities = {
    ...
    ["Shreadcc"] = function()
        ...
        then ni.spell.cast (spells.Shred.id)
    ...
    if enabled then
        ...
        for i = 1, #enemies do
            ...
            then ni.spell.cast(spells.Cyclone.id, target)
            end
        end
        end
    end
    ...
    ["CycloneFocus"] = function()
        ...
        if enabled then
            ...
            and ni.spell.valid("focus", spells.Cyclone.id, false, true)
                then ni.spell.cast(spells.Cyclone.id, "socus")
                end
            end
        end,
    ...
    ["DispelHEX"] = function()
        for i = 1, #ni.members do
            ...
            then ni.spell.cast(spells.RemoveCurse.id, ni.members[i].unit)
            end
        end
    end,
    ["Hibernate"] = function()
        ...
        if ni.player.buff (spells.InstantCast.id) then
            ...
            if ni.unit.buffs(target, 769 or 9634 or 26445)
                then ni.spell.cast(spells.Hibernate.id, target)
                end
            end
        end
    end,
    ["Shredauto"] = function()
        ...
        if enabled then
            ...
            and GetComboPoints("player", "target")  < 5 then 
                if ni.unit.isbehind("player", "target")
                and ni.spell.available(spells.Shred.id)
                then ni.spell.cast(spells.Shred.id, "target")

                elseif  ni.spell.available (spells.Manglecat.id)
                    and ni.player.powerraw("energy") >=50 
                then ni.spell.cast(spells.Manglecat.id, "target")
            end
        end
        end
        end,
	...
	if enabled then
        ...
        then ni.spell.cast(spells.SavageRoar.id, "target")
        end
    end
    end,
    ["Rip"] = function()
        ...
        if enabled then
            ...
            then ni.spell.cast(spells.Rip.id, "target")
            end
        end
    end,
    ...
    ["MangleBear"] = function()
        ...
        and ni.spell.available(spells.MangleBear.id)
            then ni.spell.cast(spells.MangleBear.id, "Target") 
        end
    end,
    ["Demoralazing"] = function()
        ...
        then ni.spell.cast(spells.Demoralazing.id)
        end
    end,
    ["Maul"] = function()
        if ni.player.buff (spells.BearForm.id) 
		...
		and not IsCurrentSpell(spells.Maul.id)
            then ni.spell.cast(spells.Maul.id) 
        end
    end,
	["Lacerate"] = function()
        if ni.player.buff (spells.BearForm.id) 
    ...
    ["FaerieFirebear"] = function()
        ...
        and ni.unit.exists("target")
        and UnitCanAttack("player", "target")
        then ni.spell.cast (spells.FaerieFire.id, "target")
        end
    end,
