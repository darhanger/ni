...
local abilities = {
	...
	["RakeCat"] = function()
		if
			cache.form == ss.cat and ValidUsable(spells.RakeCat.id, t) and
				ni.unit.debuffremaining(t, spells.RakeCat.id, p) < 2 and
	...
	["MangleCat"] = function()
		if cache.form == ss.cat and cache.combatpoints < 5 and ValidUsable(spells.MangleCat.id, t) and FacingLosCast(spells.MangleCat.name, t) then
			return true
		end
	end,
	["ClawCat"] = function()
		if cache.form == ss.cat and not IsSpellKnown(spells.MangleCat.id) and cache.combatpoints < 5 and ValidUsable(spells.ClawCat.id, t) and FacingLosCast(spells.ClawCat.name, t) then
			return true
		end
	end,
	["FerociousBiteCat"] = function()
		if cache.form == ss.cat and cache.combatpoints == 5 and ValidUsable(spells.FerociousBiteCat.id, t) and FacingLosCast(spells.FerociousBiteCat.name, t) then
			return true
		end
	end,
	--#region No Forms
