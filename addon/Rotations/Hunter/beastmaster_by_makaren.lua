...
ni.bootstrap.profile("beastmaster_by_makaren", queue, abilities, OnLoad, OnUnLoad);
elseif t4 > 1 then
    local enables = {
        ["target"] = true,
        ["duh"] = true,
...
cache.p = false
    ...
    local items = {
        settingsfile = "beastmaster_by_makaren.json",
        callback = GUICallback,
...
Работает в связке с функцией TTD(Time To Die)", enabled = true, value = 30, min = 1, max = 100, step = 1, key = "kamen" },
    ...
    local function OnLoad()
        ni.GUI.AddFrame("beastmaster_by_makaren", items);
    end;
    local function OnUnLoad()  
        ni.GUI.DestroyFrame("beastmaster_by_makaren");
    end;
    local cache = {
        sindra = false,
        PlayerCombat = false,
...
cache.p = false
    ...
    ni.bootstrap.profile("beastmaster_by_makaren", queue, abilities);
end;