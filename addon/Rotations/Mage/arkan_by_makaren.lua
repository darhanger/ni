...
ni.bootstrap.profile("arkan_by_makaren", queue, abilities, OnLoad, OnUnLoad);
elseif t4 > 1 then
    local enables = {
        ["target"] = true,
        ["kamen"] = true,
...
|cFFFF0000=========================|r")
    ...
    local items = {
        settingsfile = "arkan_by_makaren.json",
        callback = GUICallback,
...
Работает в связке с функцией TTD(Time To Die)", enabled = true, value = 30, min = 1, max = 100, step = 1, key = "kamen" },
    ...
    local function OnLoad()
        ni.GUI.AddFrame("arkan_by_makaren", items);
    end;
    local function OnUnLoad()  
        ni.GUI.DestroyFrame("arkan_by_makaren");
    end;
    local cache = {
        IsMoving = false,
        stopcast = false,
...
|cFFFF0000=========================|r")
    ...
    ni.bootstrap.profile("arkan_by_makaren", queue, abilities);
end;