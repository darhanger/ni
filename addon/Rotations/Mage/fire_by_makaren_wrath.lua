...
local items = {
    settingsfile = "fire_4T4_by_makaren.json",
    callback = GUICallback,
    { type = "title", text = "Fire 4T4 by |c0000CED1Makaren|r" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("fire_4T4_by_makaren", items);
end;
local function OnUnLoad()  
	ni.GUI.DestroyFrame("fire_4T4_by_makaren");
end;
local cache = {
    IsMoving = false,
    stopcast = false,
...
local spells = {
    og_gl = GetSpellInfo(42891),
    stolb = GetSpellInfo(42926),
    bomba = GetSpellInfo(55360),
    ...
    vozgr = GetSpellInfo(11129),
    vzriv = GetSpellInfo(42873),
    ozog = GetSpellInfo(42859),
...
|cFFFF0000=========================|r")
   ...
   -----------------------------------
    ...
    ["og_gl"] = function()
        ...
        and ni.player.buff(48108)
        and ni.spell.available(spells.og_gl)
        and ni.spell.valid("target", spells.og_gl, false, true, true) then
            ni.spell.cast(spells.og_gl)
            cache.dc = true
        end
    end,
    -----------------------------------
    ["stolb"] = function()
        ...
        and ni.player.buff(304764)
        and ni.unit.debuffremaining("target", 304765, "player") < 1
        and ni.spell.available(spells.stolb)
        and ni.spell.valid("target", spells.bomba, false, true, true) then
            ...
            if ni.spell.available(spells.vozgr) then
                ni.spell.cast(spells.vozgr)
            end
            cache.dc = false
            ni.spell.castat(spells.stolb, "target")
        end
    end,
    -----------------------------------
    ["bomba"] = function()
        if cache.PlayerCombat
        and not cache.control
        and ni.spell.available(spells.bomba)
        and ni.unit.debuffremaining("target", spells.bomba, "player") < 0.1
        and ni.spell.valid("target", spells.bomba, false, true, true) then
            ni.spell.cast(spells.bomba)
            cache.dc = true
        end
    end,
    -----------------------------------
    ["vzriv"] = function()
        if cache.PlayerCombat
        and not cache.control
        and ni.spell.available(spells.vzriv)
        and ni.spell.valid("target", spells.vzriv, false, true, true) then
            ni.spell.cast(spells.vzriv)
            cache.dc = true
        end
    end,
    -----------------------------------
...
ni.bootstrap.profile("fire_by_makaren_wrath", queue, abilities, OnLoad, OnUnLoad);
elseif t4 > 1 then
    local enables = {
        ["target"] = true,
        ["kamen"] = true,
    ...
    local items = {
        settingsfile = "fire_2T4_by_makaren.json",
        callback = GUICallback,
        { type = "title", text = "Fire 2T4 by |c0000CED1Makaren|r" },
        { type = "separator" },
        { type = "title", text = "|cffFF7C0AProfile version 2.0.0|r" },
        { type = "separator" },
        { type = "page", number = 1, text = "|cffFFFF00Общие настройки|r" },
...
Работает в связке с функцией TTD(Time To Die)", enabled = true, value = 30, min = 1, max = 100, step = 1, key = "kamen" },
    ...
    local function OnLoad()
        ni.GUI.AddFrame("fire_2T4_by_makaren", items);
    end;
    local function OnUnLoad()  
        ni.GUI.DestroyFrame("fire_2T4_by_makaren");
    end;
    local cache = {
        IsMoving = false,
        stopcast = false,
    ...
    local spells = {
        ...
        vzriv = GetSpellInfo(42873),
        ozog = GetSpellInfo(42859),
        shar = GetSpellInfo(42833),
        srt = GetSpellInfo(47610),
...
|cFFFF0000=========================|r")
    ...
    ["stop"] = function()
        ...
        ["ozog"] = function()
            ...
            and not cache.stopcast
            and cache.PlayerCombat
            and ni.unit.debuffremaining("target", 22959, "player") < 0.1
            and ni.spell.available(spells.ozog)
            and ni.spell.valid("target", spells.ozog, false, true, true) then
                ni.spell.cast(spells.ozog)
            end
        ...
        ["str"] = function()
            ...
            and not cache.IsMoving
            and not cache.stopcast
            and ni.spell.available(spells.srt)
            and ni.spell.valid("target", spells.srt, false, true, true) then
                local name, iconPath, tier, column, currentRank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(3, 8);
                if isExceptional == 1 then
                    ni.spell.cast(spells.srt)
                else
                    ni.spell.cast(spells.shar)
                end
            end
        end,
        -----------------------------------
    ...
    local items = {
        settingsfile = "fire_by_makaren_wrath.json",
        callback = GUICallback,
        { type = "title", text = "Fire by |c0000CED1Makaren|r" },
        { type = "separator" },
        { type = "title", text = "|cffFF7C0AProfile version 2.0.0|r" },
        { type = "separator" },
        { type = "page", number = 1, text = "|cffFFFF00Общие настройки|r" },
...
Работает в связке с функцией TTD(Time To Die)", enabled = true, value = 30, min = 1, max = 100, step = 1, key = "kamen" },
    ...
    local function OnLoad()
        ni.GUI.AddFrame("fire_by_makaren_wrath", items);
    end;
    local function OnUnLoad()  
        ni.GUI.DestroyFrame("fire_by_makaren_wrath");
    end;
    local cache = {
        IsMoving = false,
        stopcast = false,
...
|cFFFF0000=========================|r")
    ...
    ni.bootstrap.profile("fire_by_makaren_wrath", queue, abilities);
end;