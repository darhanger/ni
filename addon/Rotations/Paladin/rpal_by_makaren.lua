...
|cFFFF0000=========================|r")
    ...
    ni.bootstrap.profile("rpal_by_makaren", queue, abilities, OnLoad, OnUnLoad);
elseif t4 > 1 then
    local enables = {
        ["target"] = true,
        ["kamen"] = true,
...
|cFFFF0000=========================|r")
    ...
    local items = {
        settingsfile = "rpal_by_makaren.json",
        callback = GUICallback,
...
{ type = "separator" },
    ...
    local function OnLoad()
        ni.GUI.AddFrame("rpal_by_makaren", items);
    end;
    local function OnUnLoad()  
        ni.GUI.DestroyFrame("rpal_by_makaren");
    end;
    local cache = {
        sindra = false,
        PlayerCombat = false,
...
|cFFFF0000=========================|r")
    ...
    ni.bootstrap.profile("rpal_by_makaren", queue, abilities);
end;