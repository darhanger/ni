...
ni.bootstrap.profile("adk_by_makaren", queue, abilities, OnLoad, OnUnLoad);
elseif t4 > 1 then
    local enables = {
        ["target"] = true,
        ["shild"] = true,
...
|cFFFF0000=========================|r")
    ...
    local items = {
        settingsfile = "adk_by_makaren.json",
        callback = GUICallback,
...
Работает в связке с функцией TTD(Time To Die)", enabled = true, value = 30, min = 1, max = 100, step = 1, key = "kamen" },
    ...
    local function OnLoad()
        ni.GUI.AddFrame("adk_by_makaren", items);
    end;
    local function OnUnLoad()  
        ni.GUI.DestroyFrame("adk_by_makaren");
    end;
    local cache = {
        sindra = false,
        PlayerCombat = false,
...
|cFFFF0000=========================|r")
    ...
    ni.bootstrap.profile("adk_by_makaren", queue, abilities);
end;