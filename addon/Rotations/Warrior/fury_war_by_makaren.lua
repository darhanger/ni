...
ni.bootstrap.profile("fury_war_by_makaren", queue, abilities, OnLoad, OnUnLoad);
elseif t4 > 1 then
    local enables = {
        ["shild"] = true,
        ["target"] = true,
...
|cFFFF0000=========================|r")
    ...
    local items = {
        settingsfile = "fury_war_by_makaren.json",
        callback = GUICallback,
...
Работает в связке с функцией TTD(Time To Die)", enabled = true, value = 30, min = 1, max = 100, step = 1, key = "kamen" },
    ...
    local function OnLoad()
        ni.GUI.AddFrame("fury_war_by_makaren", items);
    end;
    local function OnUnLoad()  
        ni.GUI.DestroyFrame("fury_war_by_makaren");
    end;
    local cache = {
        sindra = false,
        PlayerCombat = false,
