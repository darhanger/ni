...
local spells = {--ni.vars.build == 40300
--General
...
local abilities = {
	...
	["VictoryRush"] = function ()
		if ValidUsable(spells.VictoryRush.id,t) and IsUsableSpell(spells.VictoryRush.name) and FacingLosCast(spells.VictoryRush.name, t) then
			return true
		end
	end,
	["Rend"] = function ()
	...
	["Overpower"] = function ()
		if ValidUsable(spells.Overpower.id,t) and IsUsableSpell(spells.Overpower.name) and FacingLosCast(spells.Overpower.name, t) then
			return true
		end
	end,
	["HeroicStrike"] = function ()
		if ValidUsable(spells.HeroicStrike.id,t) and
		(ni.player.buff(BattleTrance) or cache.rage >= values["HeroicStrike"]) and
		FacingLosCast(spells.HeroicStrike.name, t) then
			return true
		end
	end,
	["Slam"] = function ()
		if ValidUsable(spells.Slam.id,t) and
		(cache.rage >= values["Slam"]) and
		FacingLosCast(spells.Slam.name, t) then
			return true
		end
	end,
	["ThunderClap"] = function ()
		if ni.spell.available(spells.ThunderClap.id) and
		cache.activeenemies >= 3 and
		cache.rage >= 30 then
