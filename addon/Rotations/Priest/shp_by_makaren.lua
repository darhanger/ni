...
Работает в связке с функцией TTD(Time To Die)", enabled = true, value = 30, min = 1, max = 100, step = 1, key = "kamen" },
    ...
    { type = "separator" },
    { type = "entry", text = ni.spell.icon(988).." Диспел", tooltip ="Диспелит все что может", enabled = false, key = "decurse" },
    { type = "entry", text = ni.spell.icon(32375).." Использовать масс диспел", tooltip ="Использует масс диспел при количестве игроков", enabled = false,value = 2, min = 2, max = 10, step = 1, width = 40,  key = "decurse_3" },
    ...
    { type = "separator" },
    { type = "entry", text = ni.spell.icon(34433).." Авто пет", tooltip ="Автоматически вызывает пета при мане меньше", enabled = true, value = 60, min = 1, max = 90, step = 1, key = "pet" },
    { type = "entry", text = ni.spell.icon(47585).." Авто туча", tooltip ="Автоматически использует тучу при мане меньше", enabled = true, value = 50, min = 1, max = 80, step = 1, key = "tucha" },
    { type = "separator" },
    { type = "entry", text = ni.spell.icon(316466).." Фрагмент души по кд", tooltip ="Постоянно поддерживает баф |cFFFF0000В выключеном режиме используется только под прок перчаток или плаща|r", enabled = true, key = "fd" },
    { type = "separator" },
    { type = "entry", text = ni.spell.icon(53023).." Авто-АОЕ", tooltip = "Использует иссушение при количестве противников равному", enabled = true, value = 3, min = 3, max = 10, step = 1, width = 40, key = "aoe" },    
};
...
ni.bootstrap.profile("shp_by_makaren", queue, abilities, OnLoad, OnUnLoad);
elseif t4 > 1 then
    local enables = {
        ["target"] = true,
        ["pet"] = true,
...
|cFFFF0000=========================|r")
    ...
    local items = {
        settingsfile = "shp_by_makaren.json",
        callback = GUICallback,
        { type = "title", text = "shp_4t4 by |c0000CED1Makaren|r" },
	{ type = "separator" },
	{ type = "title", text = "|cffFF7C0AProfile version 2.0.0|r" },
...
Работает в связке с функцией TTD(Time To Die)", enabled = true, value = 30, min = 1, max = 100, step = 1, key = "kamen" },
    ...
    local function OnLoad()
        ni.GUI.AddFrame("shp_by_makaren", items);
    end;
    local function OnUnLoad()  
        ni.GUI.DestroyFrame("shp_by_makaren");
    end;
    local cache = {
        IsMoving = false,
        stopcast = false,
...
|cFFFF0000=========================|r")
    ...
    ni.bootstrap.profile("shp_by_makaren", queue, abilities);
end;