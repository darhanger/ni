...
if b == 30300 and a then
    ...
    local A = {
        ...
        ["Perchi"] = function()
            if
                ni.player.slotcastable(10) and ni.player.slotcd(10) == 0 and ni.unit.isboss("target") and
                    ni.spell.valid("target", y.SinisterStrike, true, true)
             then
        ...
        ["Kick"] = function()
            ...
            if D then
                if ni.unit.iscasting("target") or ni.unit.ischanneling("target") then
                    if
                        ni.spell.available(y.Kick) and a.spellusable(y.Kick) and ni.spell.getinterrupt("player") and
                            (ni.unit.castingpercent("target") >= C or ni.unit.channelpercent("target") >= C) and
                            ni.spell.valid("target", y.Kick, true, true)
                     then
            ...
            if C == 1 then
                if G then
                    if UnitExists(G.unit) then
                        if ni.spell.valid(G.unit, y.TricksOfTheTrade, false, true, true) then
                            if ni.spell.available(y.TricksOfTheTrade) then
                                ni.spell.cast(y.TricksOfTheTrade, G.unit)
                                return true
                            end
                        end
                    end
                ...
                if C == 2 then
                    if H then
                        if UnitExists(H.unit) then
                            if ni.spell.valid(H.unit, y.TricksOfTheTrade, false, true, true) then
                                if ni.spell.available(y.TricksOfTheTrade) then
                                    ni.spell.cast(y.TricksOfTheTrade, H.unit)
                                    return true
                                end
                            end
                        end
                        if C == 3 then
                            if
                                ni.unit.isboss("target") and ni.unit.exists("focus") and
                                    ni.spell.valid("focus", y.TricksOfTheTrade, false, true, true)
                             then
                                if ni.spell.available(y.TricksOfTheTrade) then
                                    ni.spell.cast(y.TricksOfTheTrade, "focus")
                                    return true
                                end
                            end
                        end
        ...
        ["BladeFlury"] = function()
            ...
            if D then
                if ni.unit.isboss("target") or ni.vars.combat.cd then
                    if
                        ni.spell.available(y.BladeFlury) and a.spellusable(y.BladeFlury) and
                            ni.spell.valid("target", y.SinisterStrike, true, true)
                     then
                        ni.spell.cast(y.BladeFlury)
                    end
                end
            end
        end,
        ["SliceAndDice"] = function()
            ...
            if D then
                if
                    GetComboPoints("player", "target") >= C and ni.spell.available(y.SliseAndDice) and
                        not ni.vars.combat.aoe and
                        ni.unit.buffremaining("player", y.SliseAndDice) <= 1 and
                        a.spellusable(y.SliseAndDice) and
                        ni.spell.valid("target", y.SliseAndDice, true, true)
                 then
                    ni.spell.cast(y.SliseAndDice, "target")
                end
            end
        end,
        ["KillingSpree"] = function()
            ...
            if D then
                if
                    (ni.unit.isboss("target") or ni.vars.combat.cd) and ni.player.buff(y.SliseAndDice) and
                        ni.spell.available(y.KillingSpree) and
                        a.spellusable(y.KillingSpree) and
                        ni.spell.valid("target", y.SinisterStrike, true, true)
                 then
                    ni.spell.cast(y.KillingSpree)
                end
            end
        end,
        ["Finisher"] = function()
            ...
            if I == 1 then
                if GetComboPoints("player", "target") >= C then
                    if
                        ni.spell.available(y.Rupture) and a.spellusable(y.Rupture) and
                            not ni.unit.debuff("target", y.Rupture, "player") and
                            ni.spell.valid("target", y.Rupture, true, true)
                     then
                        ni.spell.cast(y.Rupture, "target")
                    else
                        if ni.unit.debuff("target", y.Rupture, "player") then
                            if
                                GetComboPoints("player", "target") >= C and ni.spell.available(y.Eviscrate) and
                                    a.spellusable(y.Eviscrate) and
                                    ni.unit.debuffremaining("target", y.Rupture, "player") >= 3 and
                                    ni.spell.valid("target", y.Eviscrate, true, true)
                             then
                                ni.spell.cast(y.Eviscrate, "target")
                            end
                        end
                    end
                end
            ...
            if I == 2 then
                if
                    GetComboPoints("player", "target") >= C and ni.spell.available(y.potr) and a.spellusable(y.potr) and
                        ni.spell.valid("target", y.potr, true, true)
                 then
                    ni.spell.cast(y.potr, "target")
                    return true
                end
            end
        end,
        ["BladeFan"] = function()
            if ni.vars.combat.aoe then
                if
                    ni.spell.available(y.BladeFan) and a.spellusable(y.BladeFan) and
                        a.getrange("target", y.SinisterStrike) and
                        ni.spell.valid("target", y.SinisterStrike, true, true)
                 then
                    ni.spell.cast(y.BladeFan, "target")
                end
            end
        end,
        ["SinisterStrike"] = function()
            if
                a.spellusable(y.SinisterStrike) and ni.spell.available(y.SinisterStrike) and
                    ni.spell.valid("target", y.SinisterStrike, true, true)
             then
                ni.spell.cast(y.SinisterStrike, "target")
            end
        end,
        ["AdrenalineRush"] = function()
            ...
            if D then
                if ni.unit.isboss("target") or ni.vars.combat.cd then
                    if ni.spell.available(y.AdrenalineRush) and a.spellusable(y.AdrenalineRush) then
                        ni.spell.cast(y.AdrenalineRush)
                    end
                end
            end
        end,
        ["CloakOfShadows"] = function()
            ...
            if D then
                if ni.player.hp() <= C and a.spellusable(y.CloakOfShadows) and ni.spell.available(y.CloakOfShadows) then
                    ni.spell.cast(y.CloakOfShadows)
                end
            end
        end,
        ["Vanish"] = function()
            ...
            if D then
                if ni.unit.threat("player", "target") >= 2 then
                    if ni.spell.available(y.Vanish) and #ni.members >= 2 and a.spellusable(y.Vanish) then
                        ni.spell.cast(y.Vanish)
                    end
                end
            end
        end
