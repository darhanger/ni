...
local items = {
    settingsfile = "subtlety_by_makaren.json",
...
local function OnLoad()
	ni.GUI.AddFrame("subtlety_by_makaren", items);
end;
local function OnUnLoad()  
	ni.GUI.DestroyFrame("subtlety_by_makaren");
end;
local cache = {
    PlayerCombat = false,
    aoe = false,
...
|cFFFF0000=========================|r")
	...
	ni.bootstrap.profile("subtlety_by_makaren", queue, abilities, OnLoad, OnUnLoad);
else
	local queue = {
        "Error",
    };
    ...
    ni.bootstrap.profile("subtlety_by_makaren", queue, abilities);
end;