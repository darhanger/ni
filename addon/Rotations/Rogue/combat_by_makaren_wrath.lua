...
local items = {
    settingsfile = "combat_4T4_by_makaren.json",
    callback = GUICallback,
    { type = "title", text = "Combat 4T4 by |c0000CED1Makaren|r" },
	{ type = "separator" },
...
local function OnLoad()
	ni.GUI.AddFrame("combat_4T4_by_makaren", items);
end;
local function OnUnLoad()  
	ni.GUI.DestroyFrame("combat_4T4_by_makaren");
end;
local cache = {
    PlayerCombat = false,
    aoe = false,
...
local spells = {
    ...
    chereda = GetSpellInfo(51690),
    potrosh = GetSpellInfo(48668),
    kovar = GetSpellInfo(48638),
...
|cFFFF0000=========================|r")
    ...
    ["chereda"] = function()
        ...
        and ni.spell.available(spells.chereda)
        and ni.unit.distance("player", "target") < 10 then
            ni.spell.cast(spells.chereda)
    ...
    ["kovar"] = function()
        ...
        and not cache.control
        and cache.aoe == false
        and ni.spell.available(spells.kovar)
        and ni.spell.valid("target", spells.kovar, false, true, true) then
            ni.spell.cast(spells.kovar)
        end
    end,
    
    -----------------------------------
};
ni.bootstrap.profile("combat_by_makaren_wrath", queue, abilities, OnLoad, OnUnLoad);
elseif t4 > 1 then
    local enables = {
        ["target"] = true,
        ["kamen"] = true,
...
|cFFFF0000=========================|r")
    ...
    ni.bootstrap.profile("combat_by_makaren_wrath", queue, abilities);
end;